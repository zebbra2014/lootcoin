...
#if defined(NDEBUG)
# error "LootCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x93871b5721ef9bf8e45a33be4f7b7c7a866ef055f044904bcdaae8d63ef45f9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LootCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LootCoin Signed Message:\n";
...
{
    // LootCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 2157895 * COIN;
            }

    // Subsidy is cut in half every 410000 blocks
    nSubsidy >>= (nHeight / 410000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LootCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LootCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1456336207, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "240216 May Mining Never Die";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0404a0cb5d3c103916c85a5a061b2827439a88606e07d76661215d1c1e865269cf5f2ed232da4c1dbcd6da62b7482792d509e620fb3090f386abdc50088a71e0e3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1456336207;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1143780;
        ...
        assert(block.hashMerkleRoot == uint256("0x53fb82f223abd72b2f604ab650fb10f26c06e5875e96d7676918f1249d5ff621"));
...
unsigned char pchMessageStart[4] = { 0x98, 0x24, 0x8d, 0x17 }; // LootCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LootCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LootCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LootCoinMiner(CWallet *pwallet)
{
    printf("LootCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lootcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LootCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LootCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LootCoinMiner, pwallet));
}
